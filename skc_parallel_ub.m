...
for depth = 1:final_depth
    if depth == 1
        ...
        for key_ind = 1:lk
            ...
            [ub, flag] = ubfunction(x,y,gp_var_best,precond);
            if flag > 0
                fprintf([key ': PCG did not converge fully \n'])
            end
            ...
            elseif ub < kernel_top.lb % kernel interval strictly below top kernel interval
                % ignore kernel
            elseif ub < kernel_top.ub % kernel interval overlaps with top kernel interval, but has lower ub than top_kernel
                [buffer_min_val, buffer_min_ind] = findmin(kernel_buffer);
                if n_buffer < S % buffer not full
                    kernel_buffer(n_buffer+1) = kernel;
                elseif ub > buffer_min_val % if kernel has higher lb than some kernel in buffer
                    kernel_buffer(buffer_min_ind) = kernel;
                end
            else % kernel.ub > kernel_top.ub
                kernel_top = kernel;
                ...
                else % buffer full, so replace the buffer kernel with the lowest ub
        ...
        if isempty(kernel_new) % no new kernels found in search
            ...
            for key_ind = 1:lk
                ...
                elseif ub < kernel_top.ub % kernel interval overlaps with top kernel interval, but has lower ub than top_kernel
                    ...
                    if n_buffer < S % buffer not full
                        kernel_buffer(n_buffer+1) = kernel;
                    elseif ub > buffer_min_val % if kernel has higher ub than some kernel in buffer
                        kernel_buffer(buffer_min_ind) = kernel;
                    end
                else % kernel.ub > kernel_top.ub
                    kernel_top = kernel;
...
function [min_val,min_ind] = findmin(buffer) % find min_ind, min_val of struct array of kernels
    ...
    buffer_ub = zeros(1,n_buffer);
    for buffer_ind = 1:n_buffer % extract the ub of kernels in val into
        buffer_ub(buffer_ind) = buffer(buffer_ind).ub;
    end
    [min_val,min_ind] = min(buffer_ub);
end
