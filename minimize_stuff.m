function [X, fX, i, flag] = minimize_stuff(X, f, length, varargin)
...
flag=0; %becomes 1 if algorithm halts due to hyperparams becoming too large
...
if flag, warning('Optimisation halted due to hyperparams becoming extreme. Try different initialisations'); end
X = rewrap(Z,X); 
fprintf('\n'); if exist('fflush','builtin') fflush(stdout); end
