...
if isnan(fold)
    fprintf('Initial function value is NaN, so optim stopped \n') %%% added by hjk
    fval = fold; %%% added by hjk
    return %%% added by hjk
end
if isnan(gradold)
  fprintf('Initial gradient is NaN, so optim stopped \n') %%% added by hjk
  fval = fold; %%% added by hjk
  return %%% added by hjk
end
funcCount=1;
...
while (j <= maxiter)
  ...
  if (success == 1)
    ...
    xplus = x + sigma*d;
    [tmp,gplus] = fun(xplus);
    numiter=1; %%% Added by hjk
    while ((any(isinf(gplus)) || any(isnan(gplus)))) && ~isnan(fold)
      sigma=2*sigma;
      kappa=0.5.^2*kappa;
      xplus=x+sigma*d;
      [tmp,gplus] = fun(xplus);
      if numiter > 50 %%% Added by hjk
          fprintf('Optimiser is stuck \n');
        return
      end
      numiter=numiter+1; %%% Added by hjk
    end
  ...
  if (nsuccess == nparams)
    ...
    if (success == 1)
      ...
      %%% added by hjk (can get error 'Inner matrix dimensions must agree.')
      try
        d = beta*d - gradnew;
      catch
        fprintf('Inner matrix dimensions must agree, so breaking \n')
        break
      end
    end
  end
