...
%       edata = 0.5*MNM + logK + 0.5*log(det(B)) + 0.5*log(det(A)) + 0.5*n*log(2*pi);
  ...
  case {'DTC' 'VAR' 'SOR'}
    ...
    K_uu = gp_trcov(gp, u);          % m x m, noiseles covariance K_uu
    K_uu = (K_uu+K_uu')./2;          % ensure the symmetry of K_uu
    [Luu, notpositivedefinite] = chol(K_uu, 'lower'); % L_uu*L_uu' = K_uu
    if notpositivedefinite
      [edata, eprior, e] = set_output_for_notpositivedefinite;
      return
    end
...
%%%%%%%%%%%%%%%%%%%%%%
% above modified to (more numerically stable, but only for homoscedactic noise for now)
    signal_var = Lav(1);
    m = size(u,1);
    ...
    b=(((y'*K_fu)/(Luu'))/LA)/signal_var;
    edata = 0.5*sum(log(Lav)) + 0.5*y'./Lav'*y + sum(log(diag(LA))) ...
        - 0.5*sum(b.^2)+0.5*n*log(2*pi);
