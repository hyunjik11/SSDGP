function [x,flag,relres,iter,resvec,obj] = cgs_obj(A,b,tol,maxit,M1,M2,x0,varargin)
%CGS   Conjugate Gradients Squared Method.
%%%% obj added by hjk: returns vector of objective values x'*A*x/2-x'*b
...
maxstagsteps = 3;
obj = zeros(maxit+1,1);            %%%% Added by hjk: value of objective
obj(1) = -sum(x.*(r+b))/2;         %%%% Added by hjk 
...
for ii = 1 : maxit
    ...
    normr_act = normr;
    resvec(ii+1) = normr;
    obj(ii+1) = -sum(x.*(r+b))/2;  %%%% Added by hjk: value of objective
...
if flag <= 1 || flag == 3
    ...
    obj = obj(1:ii+1);          %%%% Added by hjk
else
    resvec = resvec(1:ii);
    obj = obj(1:ii);            %%%% Added by hjk
end
