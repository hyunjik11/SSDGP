function [e, edata, eprior] = gp_f(w, gp, x, y, varargin)
...
%       edata = 0.5*MNM + logK + 0.5*log(det(B)) + 0.5*log(det(A)) + 0.5*n*log(2*pi);
  ...
  case {'DTC' 'VAR' 'SOR'}
    ...
    % First some help matrices...
    % A = chol(K_uu+K_uf*inv(Lav)*K_fu))
    A = K_uu+K_fu'*iLaKfu;
    A = (A+A')./2;% + 1e-8*eye(size(A,1));     % Ensure symmetry
    [A, notpositivedefinite] = chol(A);
    if notpositivedefinite
        % instead of working with A=K_uu+K_uf*inv(Lav)*K_fu)
        % work with A=sigma2*K_uu+K_uf*K_fu, which is normally more stable with chol
        % Here we assume homoscedacity i.e. Lav=sigma2*ones
        sigma2=Lav(1);
        A = sigma2*K_uu+K_fu'*K_fu;
        A = (A+A')./2 + 1e-12*eye(size(A,1)); %add jitter for stable chol
        [A, notpositivedefinite] = chol(A);
        if notpositivedefinite
            [edata, eprior, e] = set_output_for_notpositivedefinite;
            return
            ...
            edata = (n-m)*log(sigma2) + sum(y.^2)/sigma2 - 2*sum(log(diag(Luu))) + 2*sum(log(diag(A))) - sum(b.^2)/sigma2; 
